mainApp.controller("masterRewardCtrl", function ($route, $scope, $uibModal, $routeParams, $q, $cookies, Constant, HttpRequest, Model, Helper, DTOptionsBuilder, DTColumnBuilder, markers, Upload, $timeout) {
    //Variable
        ...
        HttpRequest.post(apiUrl, dataForm).success(function (response) {
            ...
            if (file == undefined) {
                ...
                file.upload = Upload.upload({
                    url: 'https://api.myerlinski.com/reward/uploadReward',
                    // url: '',
                    data: {
                        id: $scope.idBarang,
                        file: file
    ...
    $scope.eventClickUpdate = function (file) {
        $('.Loading').show();
        $('.page-form').hide();
        var apiUrl = "/reward/" + $scope.form.id;
