mainApp.controller("klaimRewardCtrl", function ($scope, $sce, $routeParams, $q, $cookies, Constant, HttpRequest, Model, Helper, DTOptionsBuilder, DTColumnBuilder) {
    ...
    $scope.tanggalSekarang = $scope.dateNow;
    //Procedures =====================================================================================================================
    ...
    $scope.renderList = function () {
        $('.Loading').show();
        $('.page-form').hide();
        var apiUrl = "/reward/" + $scope.currentUser.email + "/Paket/getRewardByJenis";
        HttpRequest.get(apiUrl).success(function (response) {
            $scope.listDataPaket = response.data;
            console.log(JSON.stringify($scope.listDataPaket));
            $('.Loading').hide();
            $('.page-form').show();

        })
        var apiUrlSatuan = "/reward/" + $scope.currentUser.email + "/Satuan/getRewardByJenis";
        HttpRequest.get(apiUrlSatuan).success(function (response) {
            $scope.listDataSatuan = response.data;
            console.log(JSON.stringify($scope.listDataPaket));
    ...
    $scope.klaimRewardPaket = (idReward, jenisReward, poin, namaReward) => {
        var dataForm = {
            "tanggal": $scope.tanggalSekarang,
            "idMember": $scope.currentUser.email,
            "idReward": idReward,
        ...
        swal({
                title: "Info!!!",
                text: "Apakah Anda Yakin Ingin Klaim Reward Ini?",
                icon: "warning",
                buttons: true,
                successMode: true,
            })
            .then((willOk) => {
                ...
                $('.page-form').hide();
                if (willOk) {
                    var apiUrl = "/klaim/create";
                    HttpRequest.post(apiUrl, dataForm).success(function (response) {
                        swal("Selamat Anda Mendapatkan Hadiah " + namaReward, "Silahkan Hub. Erlinski Official", "success");
                        $scope.renderList();
                    })
    ...
    $scope.klaimRewardSatuan = (idReward, jenisReward, poin, namaReward) => {
        var dataForm = {
            "tanggal": $scope.tanggalSekarang,
            "idMember": $scope.currentUser.email,
            "idReward": idReward,
